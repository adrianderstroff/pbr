...
// It can be bound as alternative render target contrary to the default frame
// buffer.
type FBO struct {
	handle        uint32
	isBound       bool
...
// GetColorTexture returns the color texture at the position of the specified
// index.
...
// Make creates an FBO with one color and depth texture of the specified width
// and height.
...
// MakeMultisample make a multisampled frame buffer with a color and a depth
// texture of the specified width, height and samples attached.
...
func (fbo *FBO) AttachColorTexture(texture *tex.Texture, index uint32) {
	...
	gl.FramebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0+index,
		fbo.textureType, texture.GetHandle(), 0)
	fbo.drawbuffers = append(fbo.drawbuffers, gl.COLOR_ATTACHMENT0+index)
	gl.DrawBuffers(int32(len(fbo.drawbuffers)), &fbo.drawbuffers[0])
	fbo.Unbind()
...
// IsComplete if the framebuffer is complete
...
// CopyToScreenRegion copies all color and depth textures within a region
// specified by the position (x1,y1) and width w1 and height h1 to the default
// frame buffer in the region (x2,y2) and the width w2 and height h2.
...
// CopyColorToScreen copies all color textures to the default frame buffer.
func (fbo *FBO) CopyColorToScreen(index uint32, x, y, width, height int32) {
	fbo.CopyColorToScreenRegion(index, x, y, width, height, x, y, width, height)
}

// CopyColorToScreenRegion copies all color textures within a region specified
// by the position (x1,y1) and width w1 and height h1 to the default frame
// buffer in the region (x2,y2) and the width w2 and height h2.
func (fbo *FBO) CopyColorToScreenRegion(index uint32, x1, y1, w1, h1, x2, y2, w2, h2 int32) {
	gl.BindFramebuffer(gl.DRAW_FRAMEBUFFER, 0)
	gl.DrawBuffer(gl.BACK)
	gl.BindFramebuffer(gl.READ_FRAMEBUFFER, fbo.handle)
	gl.ReadBuffer(gl.COLOR_ATTACHMENT0 + index)
...
// CopyDepthToScreen copies the depth texture to the default frame buffer.
func (fbo *FBO) CopyDepthToScreen(x, y, width, height int32) {
	fbo.CopyDepthToScreenRegion(x, y, width, height, x, y, width, height)
}

// CopyDepthToScreenRegion copies the depth texture within a region specified
// by the position (x1,y1) and width w1 and height h1 to the default frame
// buffer in the region (x2,y2) and the width w2 and height h2.
func (fbo *FBO) CopyDepthToScreenRegion(x1, y1, w1, h1, x2, y2, w2, h2 int32) {
	gl.BindFramebuffer(gl.DRAW_FRAMEBUFFER, 0)
	gl.DrawBuffer(gl.BACK)
	gl.BindFramebuffer(gl.READ_FRAMEBUFFER, fbo.handle)
...
// CopyToFBORegion copies all color and depth textures within a region
// specified by the position (x1,y1) and width w1 and height h1 to another FBO
// in the region (x2,y2) and the width w2 and height h2.
...
// CopyColorToFBORegion copies all color textures within a region specified by
// the position (x1,y1) and width w1 and height h1 to another FBO in the region
// (x2,y2) and the width w2 and height h2.
...
// CopyColorToFBOSmooth copies all color textures to another FBO using linear
// interpolation.
...
// CopyColorToFBORegionSmooth copies all color textures within a region
// specified by the position (x1,y1) and width w1 and height h1 to another FBO
// in the region (x2,y2) and the width w2 and height h2 using linear
// interpolation.
...
// CopyAttachmentColorToFBO copies a color texture specified by index1 to the
// color texture of another FBO at index2.
...
// CopyColorAttachmentToFBORegion copies a texture specfied by index2 within a
// region specified by the position (x1,y1) and width w1 and height h1 to the
// color texture of another FBO at index2 in the region (x2,y2) and the width
// w2 and height h2 using linear interpolation.
...
// CopyAttachmentColorToFBOSmooth copies a color texture specified by index1 to
// the color texture of another FBO at index2 using linear interpolation.
...
// CopyAttachmentColorToFBORegionSmooth copies a texture specfied by index2
// within a region specified by the position (x1,y1) and width w1 and height h1
// to the color texture of another FBO at index2 in the region (x2,y2) and the
// width w2 and height h2 using linear interpolation using linear interpolation.
...
// CopyDepthToFBORegion copies the depth texture within a region specified by
// the position (x1,y1) and width w1 and height h1 to another FBO in the region
// (x2,y2) and the width w2 and height h2.
